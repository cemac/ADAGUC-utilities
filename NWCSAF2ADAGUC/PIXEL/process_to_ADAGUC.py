...
def readConf():
    '''
    Reads the configuration from the file pixel.conf
    ...
    # scanning working dir and subdirs looking for the pixel.conf file
    for name_subdir, name_dirs, all_files in os.walk(work_dir):
        for name_file in all_files:
            if name_file.endswith("pixel.conf"):
    ...
    if pixel_file_path == pixel_file_path_app and pixel_file_path != "":
        ...
        print("       To change the the settings please edit pixel.conf         ")
        print("       ........................................................")
        print("       ........................................................")
    ...
    if pixel_file_path == "" and pixel_file_path_app == "":
        ...
        print("                                                                ")
        print("       2- Or copy pixel.conf under the directory: " + work_dir)
        print("       ........................................................ ")
        print("                                                                ")
...
def obtain_pallette_p(nc_fid, variable_name, scheme="RGB", alpha=255):
    ...
    '''
    ancillaries = nc_fid.variables[variable_name].ancillary_variables
    ancillaries = ancillaries.split()

    ancillary_pal = [name for name in ancillaries if "pal" in name]

    var_pal = ancillary_pal[0]
...
def update_xml(nc_fid, product, variable_list, conf):
    ...
    layer_template = ('''\n<Layer  type = "database" >
    <FilePath  filter = "^.*\.nc$">/data/adaguc-autowms/PRODUCT/</FilePath>
    <Variable>VARIABLE</Variable>
    <Name>PRODUCT_VARIABLE</Name>
    ...
    for variable_name in variable_list:
        ...
        file = open(conf['XML']['outDir'] + os.sep + product + '.xml', 'w')
...
def process_one_file(inputFile, conf, underTM=False):


    srv.wait_until_closed(inputFile)
    src = netCDF4.Dataset(inputFile, 'r')
    product = src.getncattr('product_name')
    
    if product == 'L1SD':
        product = inputFile.split('_')[-4]
    ...
    output_file = outModDir + os.sep + product + os.sep + inputFile.split(os.sep)[-1]
    tempFile = tempDir + os.sep + product + os.sep + inputFile.split(os.sep)[-1]
    ...
    atributtes = {}
    for name_attr in src.ncattrs():
        atributtes[name_attr] = src.getncattr(name_attr)
    atributtes['history'] = 'original NWCSAF file modified to be compatible with ADAGUC'
    ...
    # and the variable
    if 'time' not in src.dimensions.keys():
    ...
    for name_dimension in src.dimensions:
        name = src.dimensions[name_dimension].name
        size = src.dimensions[name_dimension].size
        output_dataset.createDimension(name, size)

    '''
    Special (one dimension) variables creation: time, projection_definition, nx and ny
    '''
    ...
    try:
        time_string = src.getncattr('nominal_product_time')
    except:
        print("TIME FAILS IN: ", inputFile)
        unformated_time_string = inputFile.split('_')[-1].split('.')[0]
    ...
    projection = src.getncattr('gdal_projection')
    if ' +units=m' in projection:
        projection = projection + ' +units=m'
    ...
    #var_pr.inverse_flattening = (var_pr.semi_major_axis /(var_pr.semi_major_axis - var_pr.semi_minor_axis))
    ...
    for name in navigation_variables:
        ...
        output_dataset[name].setncatts(src[name].__dict__)
        if name == 'nx':
        ...
        output_dataset[name][:] = src[name][:] / h

    # Creating the raster and palette variables.

    toexclude = ['nx', 'ny', 'time', 'projection_definition', 'lat', 'lon']

    # clonning all the bidimensional variables with an additional time dimension
    for name, variable in src.variables.items():
        if (name not in toexclude):
            data = np.ma.array([src[name][:]])
            if '_FillValue' in variable.ncattrs():
                x = (output_dataset.createVariable(name, data.dtype, ('time',) + variable.dimensions, zlib=True,
                                                   fill_value=variable.getncattr('_FillValue')))
                data = ma.masked_where(data == variable.getncattr('_FillValue'), data)
            else:
                x = (output_dataset.createVariable(name, variable.datatype, ('time',) + variable.dimensions, zlib=True))

            # copy variable attributes all at once via dictionary
            attributes = src[name].ncattrs()
            for at in attributes:
                if at != '_FillValue':
                    output_dataset[name].setncatts({at: src[name].getncattr(at)})

            output_dataset[name].grid_mapping = ('projection_definition')
            output_dataset[name].coordinates = 'time ny nx'
            # writing the data
    ...
    if conf['XML']['UPDATE'] == 'true':
        ...
        update_xml(src, product, filled_variables, conf)
    ...
    time.sleep(0.1)
    srv.wait_until_unlocked(output_file)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        pixel_current_file = "S_NWC_" + sys.argv[1] + "_" + sys.argv[2] + "_" + sys.argv[3] + "_" + time_stamp + ".nc"
        ...
        print("       ........................................................")
        print("       Help:")
        print("       TM Usage: python process_to_ADAGUC.py <Satellite> <area> < YYYY-MM-DDThh:mm:ssZ>  ")
        print("       As in:      ")
        print("       python3 process_to_ADAGUC.py MSG4 Europe-VISIR  2019-02-02T13:00:00Z  ")
        print("       To reach whole in dir recursive processing, run without options: python3 process_to_ADAGUC.py")
        print("       In order to test your platform: python3 process_to_ADAGUC.py -d")
        print("       To reach this help: python3 process_to_ADAGUC.py -h")
        print("       CONFIGURATION IS AVAILABLE IN: pixel.conf FILE")
        print("       ........................................................")
        print("       Quiting.")
        quit()
        ...
        print("       WARMING: DEBUGING WITH SAMPLE FILE ./debug_data/S_NWC_CTTH_MSG4_Europe-VISIR_20190218T103000Z.nc")
        print("       ........................................................")
        print("       Output file fully readable on: " + configuration['PLATFORM']['viewer'])
        print("       To change the target platform edit pixel.conf")
        print("       ........................................................")
        debug_in_File = '/S_NWC_CTTH_MSG4_Europe-VISIR_20190218T103000Z.nc'
        ...
        print("       TM Usage: python process_to_ADAGUC.py <Satellite> <area> < YYYY-MM-DDThh:mm:ssZ>  ")
        print("       As in:      ")
        print("       python3 process_to_ADAGUC.py CTTH MSG4 Europe-VISIR  2019-02-02T13:00:00Z  ")
        print("       To reach whole in dir recursive processing, run without options: python3 process_to_ADAGUC.py")
        print("       In order to test your platform: python3 process_to_ADAGUC.py -d")
        print("       To reach this help: python3 process_to_ADAGUC.py -h")
        print("       CONFIGURATION IS AVAILABLE IN: pixel.conf FILE")
    ...
    for inputFile in in_files:
        ...
        process_one_file(inputFile, configuration)
