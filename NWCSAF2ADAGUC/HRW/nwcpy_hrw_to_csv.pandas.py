...
def process_one_file(hrw_file, conf, underTM=False):
    ...
    # loading the data in a dataframe in memory
    df = pd.DataFrame(data=data_dict)

    # and formatting
    df.dd = np.round(df.dd)
    df.dd = df.dd.astype(int)
    df.ff = np.round(df.ff, decimals=2)
    df.lat = np.round(df.lat, decimals=6)
    df.lon = np.round(df.lon, decimals=6)
    ...
    for key in conf['QUERY'].keys():
        for i in range(len(conf['QUERY'][key]) - 1):
            ...
            f.close()
            os.sync()
            number_of_winds = len(df[((df.pressure <= level0) & (df.pressure > level1))].index)
            ...
            if number_of_winds > 0:
                ...
                df[((df.pressure <= level0) & (df.pressure > level1))].to_csv(f, sep=',', index=False)
                f.close()
                os.sync()
                if not underTM:
